...
class robot:
    ...
    def local_rearrange(self):
        i = 0
        j = 0
        while (self.window_projection.count(False) > self.pattern.count(0) \
                and self.sandbox_projection.count(False) < self.slice_size) \
            or (self.window_projection.count(0) < self.pattern.count(0) \
                and self.sandbox_projection.count(False) > 0):

            want = bool(self.window_projection.count(False) > self.pattern.count(0))
            while self.window_projection[i] == want or self.window_projection[i] == self.pattern[i]:
                i += 1
            ...
            if self.window_projection[i] != self.pattern[i]:
                for j in range(i + 1, self.slice_size):
                    if self.window_projection[j] != self.pattern[j] \
                        ...
                        self.window_projection[i], self.window_projection[j] = self.window_projection[j], self.window_projection[i]
                        break
